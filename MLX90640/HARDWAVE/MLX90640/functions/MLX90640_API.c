...
//提取数据为有符号整型
void MLX90640_CalculateTo_int16(uint16_t *frameData, const paramsMLX90640 *params, float emissivity, float tr, int16_t  *result)
{
    float vdd;
    float ta;
    float ta4;
...
//------------------------- To calculation -------------------------------------    
    ...
    for( pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ...
        {    
            ...
            // 强制有符号存储，保留两位小数
						result[pixelNumber] = (int16_t)roundf(To * 100.0f);
...
// 提取数据为有符号 float 类型（单位：°C）
void MLX90640_CalculateTo_float(uint16_t *frameData, const paramsMLX90640 *params, float emissivity, float tr, float *result)
{
    float vdd;
    float ta;
    float ta4;
    ...
    for (pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ...
        if (pattern == frameData[833])
        {
            ...
            irData = irData - params->offset[pixelNumber] *
                                ...
                                (1 + params->kv[pixelNumber] * (vdd - 3.3f));
            if (mode != params->calibrationModeEE)
            {
